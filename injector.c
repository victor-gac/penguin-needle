...
// libname
//static const char *libname = "libcrypt.so";
...
int poke_text(pid_t pid, void *where, void *new_text, void *old_text,
  ...
  for (size_t copied = 0; copied < len; copied += sizeof(poke_data)) {
    ...
    if (old_text != NULL) {
      errno = 0;
      long int peek_data = ptrace(PTRACE_PEEKTEXT, pid, where + copied, NULL);
    ...
    printf("Try to map %p at %p\n", poke_data, where + copied);
...
int fprintf_process(pid_t pid) {
  ...
  */
  ...
  newregs.eax = 0x5a;                           // mmap
  newregs.ebx = mmap_arg_addr;

  struct mmap_arg_struct mmap_arg;
  mmap_arg.addr = 0;
  mmap_arg.len = PAGE_SIZE;  
  ...
  void *mmap_memory = (void *)newregs.eax;
  ...
  */
  if (newregs.eip == (long int)mmap_memory) {
    printf("successfully jumped to mmap area\n");
  } else {
    printf("unexpectedly jumped to %p\n", (void *)newregs.eip);
    goto fail;
  ...
  char* mode = dlsym(NULL, "__libc_dlopen_mode");
  //char* mode = dlsym(NULL, "printf");
  if(mode) 
    printf("function found at    %p\n", mode);
  void *their_fprintf = their_libc + ((void *)mode - our_libc);
  ...
  memmove(new_text + offset, libname, strlen(libname));
  ...
  newregs.rsi = 1;
  */
  newregs.eax = 0;                          // no vector registers are used
  newregs.ebx = (long)mmap_memory + offset;
  newregs.ecx = 1;
  ...
  poke_text(pid, (void *)newregs.eip, new_word, NULL, sizeof(new_word));
  ...
  */
  if (newregs.eip == (long)rip) {
    printf("successfully jumped back to original %%rip at %p\n", rip);
  } else {
    printf("unexpectedly jumped to %p (expected to be at %p)\n",
           (void *)newregs.eip, rip);
    goto fail;
  }
  ...
  */
  newregs.eax = 11;                // munmap
  newregs.ebx = (long)mmap_memory; // addr
  newregs.ecx = PAGE_SIZE;         // size
  if (ptrace(PTRACE_SETREGS, pid, NULL, &newregs)) {
    perror("PTRACE_SETREGS");
    goto fail;
  }
